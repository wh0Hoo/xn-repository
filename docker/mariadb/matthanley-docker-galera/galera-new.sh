...
if echo $* | grep -v -e '--skip-networking' | grep -v -e '--help'; then
  ...
  if [ -n "$SERVICE_NAME" ]; then
    ...
    if [ `getent hosts tasks.$SERVICE_NAME | wc -l` -lt $(((${CLUSTER_NODES}+1)/2)) ]; then
      ...
      sleep 1
      #if [ "${SERVICE_NAME}1" != $WSREP_NODE_NAME ]; then
      #exit 1
      #fi
    else
  echo "Cluster peers:"
